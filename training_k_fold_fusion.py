# -*- coding: utf-8 -*-
"""
Created on Sat Oct 26 13:04:56 2024
...
alpha_2d, beta_2d, gamma_2d = channel_mapping_2d.mapping_2d(
    'de_LDS', 'sub1ex1', path_data, path_ch_auto_distr, interpolation=True)
alpha_3d, beta_3d, gamma_3d = channel_mapping_3d.mapping_3d(
    'de_LDS', 'sub1ex1', path_data, path_ch_auto_distr, interpolation=True)

# normalize
alpha_2d, beta_2d, gamma_2d = alpha_2d / np.max(alpha_2d), beta_2d / np.max(beta_2d), gamma_2d / np.max(gamma_2d)
alpha_3d, beta_3d, gamma_3d = alpha_3d / np.max(alpha_3d), beta_3d / np.max(beta_3d), gamma_3d / np.max(gamma_3d)
...
# # read labels
# labels = utils.read_labels(path_labels)

# # 假设我们已将数据加载到 X 和 y 中，并进行了归一化
# X2d_tensor = torch.tensor(data_2d, dtype=torch.float32)
# X3d_tensor = torch.tensor(data_3d, dtype=torch.float32)
...
# # 使用示例
# k_fold_cross_validation(model, device, X2d_tensor, X3d_tensor, y_tensor, k_folds=5, use_sequential_split=True)